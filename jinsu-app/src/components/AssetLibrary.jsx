...
${text.slice(0, 12000)}`;
  ...
  const genFourView = async (c) => {
    ...
    try {
      ...
      for (let i = 0; i < views.length; i++) {
        ...
        const res = await generateImage({ prompt, model: "Qwen-Image", size: "768x1024", n: 1 });
  ...
  const genSceneImage = async (s) => {
    ...
    try {
      const prompt = `纯场景空镜/环境概念图，绝对禁止出现任何人物、人形、剪影、角色或拟人形象，画面里只呈现环境、建筑、自然、道具与氛围。\n根据以下场景描述，判断时代背景（古代/现代/民国/玄幻等）并生成对应环境。\n场景描述：${desc}\n要求：影视短剧空镜/场景概念图风格，高清写实，构图完整，氛围鲜明，光影细腻，色彩协调，空无一人的纯粹场景，无人物主体、无人形、无剪影、无角色、无任何与人类相关的元素，无文字、无水印、无边框。${dramaModifier(project.dramaType)}`;
      const res = await generateImage({ prompt, model: "Qwen-Image", size: "1024x1024", n: 1 });
  ...
  const genScene3D = async (s) => {
    ...
    try {
      ...
      exportSharedAssets();
      window.alert(`场景「${s.name}」3D 场景已生成并导出，可在「3D 导演台」查看。`);
    } catch (e) {
  ...
  const exportSharedAssets = () => {
    ...
    a.href = url; a.download = "jinsu-shared-assets.json"; a.click();
