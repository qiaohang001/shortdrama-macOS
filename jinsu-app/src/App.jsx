...
import { ThreePane, TaskCard, MenuBar, SettingsDialog, StatusBar, DockNav, TopBar, applyTheme, openDB, SessionProvider, BillingUI, AccountButton } from "@dual/ui";
...
import { VisualEditor } from "./components/NewScript/VisualEditor.jsx";
import { EpisodeBoard } from "./components/EpisodeBoard.jsx";
...
import { PricingGuide } from "./components/PricingGuide.jsx";
const TABS = [
  ...
  { k: "canvas", t: "无限画布", icon: "🧩" },
  { k: "stage3d", t: "3D导演台", icon: "🎭" },
...
const [showPricing, setShowPricing] = useState(false);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => db.saveProject("shortdrama", activeId, project), 400);
  ...
  const exportPackage = async () => {
    ...
    const res = await saveFile((project.title || "shortdrama") + "_工程包.json", blob);
  ...
  const exportProject = async () => {
    ...
    const { saveFile } = await import("@dual/ui");
    const res = await saveFile((project.title || "shortdrama") + ".json", blob);
  ...
  const exportCastScenes = async () => {
    ...
    const { saveFile } = await import("@dual/ui");
    const res = await saveFile((project.title || "shortdrama") + "_角色场景.json", blob);
  ...
  else if (activeTab === "video") center = <VideoGenBoard project={project} update={update} log={log} />;
  else if (activeTab === "editor") center = <VisualEditor project={project} update={update} log={log} />;
  else if (activeTab === "canvas") center = <InfiniteCanvas width="100%" height="100%" />;
  else if (activeTab === "stage3d") center = <Director3D width="100%" height="100%" />;
  else if (activeTab === "dub") center = <DevOnly title="配音模块" />;
  ...
  else center = <EpisodeBoard project={project} update={update} log={log} />;
  ...
  return (
    ...
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", overflow: "hidden", background: "var(--bg, #0b0f17)", fontFamily: "var(--font)" }}>
      ...
      <TopBar
        ...
        actions={<span style={{ display: "flex", alignItems: "center", gap: 12 }}><span style={{ color: "var(--text-muted, #5d6779)", fontSize: 12 }}>{project?.title || "未命名项目"}</span><button onClick={() => setShowPricing(true)} style={{ border: "1px solid var(--border)", borderRadius: 6, padding: "4px 10px", background: "transparent", color: "var(--text)", cursor: "pointer", fontSize: 12 }}>💰 收费标准</button><AccountButton /></span>}
      />
          ...
          <ThreePane
            ...
            left={
              <div style={{ display: "flex", flexDirection: "column", height: "100%", padding: 14 }}>
                ...
                <select style={{ ...s.select, marginTop: 10 }} value={project?.dramaType || "real"} onChange={(e) => update({ dramaType: e.target.value })}>
                  {DRAMA_TYPES.map((d) => <option key={d.key} value={d.key}>{d.emoji} {d.name}</option>)}
                </select>
      ...
      {showSettings && (
        <SettingsDialog open={showSettings} onClose={() => setShowSettings(false)} appName="烬序・影墟"
          showRag={false} theme={theme} onThemeChange={setTheme} features={[
            { key: "video", label: "视频剪辑模块", checked: featOn("video"), onToggle: () => toggleFeat("video") },
          ]} />
      )}
      {showTemplates && (
      ...
      <BillingUI />
      {showPricing && <PricingGuide onClose={() => setShowPricing(false)} />}
    </div>
    </SessionProvider>
  );
}
...
function HomePage({ projects, recentMap, activeId, allTags, activeQueue, failedQueue, onNew, onOpen, onEnter, onTogglePin, onToggleArchive, onClone, onTrash, onOpenTemplates, onOpenAssets }) {
  ...
  const [dramaType, setDramaType] = React.useState("real");
  ...
  return (
    <div style={{ padding: 32, overflowY: "auto", height: "100%", boxSizing: "border-box", color: "var(--text, #e8ecf3)" }}>
      <div style={{ maxWidth: 980, margin: "0 auto" }}>
        <img src={jinsuLogoH} alt="烬序 JINSU" style={{ height: 54, marginBottom: 18, borderRadius: 10 }} />
        <div style={{ fontSize: 13, color: "var(--accent-2, #3b82f6)", letterSpacing: 2, fontWeight: 700 }}>烬序・影墟</div>
        <h1 style={{ fontSize: 30, margin: "6px 0 10px" }}>用 AI 把创意拍成竖屏短剧</h1>
        <p style={{ fontSize: 14, color: "var(--text-secondary, #8b95a7)", lineHeight: 1.7, margin: "0 0 22px" }}>
          梗概 → 分场剧本 → 逐场生图/视频 → 无限画布排布 → 视频剪辑拼接 → 3D 导演台 → 配音导出。所有数据自动保存在本地。
        </p>

        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 4px", border: "1px solid var(--border, rgba(255,255,255,0.14))", borderRadius: "var(--radius-sm, 6px)", background: "var(--panel-2, #1c2433)" }}>
            <span style={{ fontSize: 13, color: "var(--text-secondary, #8b95a7)" }}>短剧类型</span>
            <select value={dramaType} onChange={(e) => setDramaType(e.target.value)} style={{ border: "none", background: "transparent", color: "var(--text, #e8ecf3)", fontSize: 13, outline: "none", padding: "8px 4px", cursor: "pointer" }}>
              {DRAMA_TYPES.map((d) => <option key={d.key} value={d.key}>{d.emoji} {d.name}</option>)}
            </select>
          </div>
          <button style={hp.btnPrimary} onClick={() => onNew(dramaType)}>＋ 新建空白项目</button>
          <button style={hp.btn} onClick={onOpenTemplates}>🎭 从模板新建</button>
          <button style={hp.btn} onClick={onEnter}>▶ 进入创作平台</button>
          <button style={hp.btn} onClick={onOpenAssets}>🗂 打开素材库</button>
        </div>

        {/* 快速任务看板（缺陷 §1：看不到排队任务 / 失败清单） */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 22 }}>
          <div style={hp.board}>
            <div style={hp.boardH}>⏳ 进行中任务（{activeQueue.length}）</div>
            {activeQueue.length === 0 ? <div style={hp.boardEmpty}>暂无排队 / 渲染中的任务。</div> : (
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {activeQueue.slice(0, 5).map((t) => (
                  <div key={t.id} style={hp.boardItem}>
                    <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.label}</span>
                    <span style={{ color: "var(--accent-2, #3b82f6)", fontSize: 11, flex: "0 0 auto", marginLeft: 8 }}>{t.status === "running" ? "生成中" : t.status === "paused" ? "暂停" : "排队"}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={hp.board}>
            <div style={hp.boardH}>⚠️ 最近失败（{failedQueue.length}）</div>
            {failedQueue.length === 0 ? <div style={hp.boardEmpty}>没有失败任务，一切顺利。</div> : (
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {failedQueue.slice(0, 5).map((t) => (
                  <div key={t.id} style={{ ...hp.boardItem, color: "var(--danger, #ef4444)" }} title={t.error || ""}>
                    <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.label}</span>
                    <span style={{ fontSize: 11, flex: "0 0 auto", marginLeft: 8 }}>失败</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {recentList.length > 0 && (
          <div style={{ margin: "22px 0 4px" }}>
            <h3 style={{ fontSize: 13, margin: "0 0 10px", color: "var(--text-secondary, #8b95a7)" }}>🕘 最近打开</h3>
            <div style={{ display: "flex", gap: 12, overflowX: "auto", paddingBottom: 4 }}>
              {recentList.map((p) => (
                <div key={p.id} onClick={() => onOpen(p.id)} style={{ flex: "0 0 auto", width: 160, cursor: "pointer", border: "1px solid var(--border, rgba(255,255,255,0.08))", borderRadius: 12, overflow: "hidden", background: "var(--panel, #161d2a)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, margin: "26px 0 12px", flexWrap: "wrap" }}>
          ...
          <input style={hp.search} placeholder="搜索项目名 / 标签…" value={q} onChange={(e) => setQ(e.target.value)} />
          <button style={{ ...hp.chip, ...(tag === "all" ? hp.chipOn : {}) }} onClick={() => setTag("all")}>全部</button>
          {allTags.map((t) => (
            <button key={t} style={{ ...hp.chip, ...(tag === t ? hp.chipOn : {}) }} onClick={() => setTag(t)}>{t}</button>
          ))}
        </div>

        {filtered.length === 0 ? <div style={hp.ph}>没有匹配的项目。点上方「新建空白项目」或「从模板新建」开始。</div> : (
              ...
              <div key={p.id} style={{ ...hp.card, ...(p.id === activeId ? hp.cardActive : {}) }}>
                ...
                <div style={{ padding: 10 }}>
                  <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginBottom: 8 }}>
                    {(p.tags || []).slice(0, 3).map((t) => <span key={t} style={hp.tag}>{t}</span>)}
                  </div>
                  <div style={{ display: "flex", gap: 6 }}>
                    ...
                    <button style={hp.iconBtn} title="归档" onClick={() => onToggleArchive(p.id)}>📥</button>
                    <button style={hp.iconBtnDanger} title="移入回收站" onClick={() => onTrash(p.id)}>🗑</button>
                  </div>
                </div>
              </div>
            ))}
        ...
        <p style={{ fontSize: 12, color: "var(--text-muted, #5d6779)", marginTop: 20 }}>提示：归档的项目不在首页显示；回收站可恢复误删工程。从顶部「素材库」可拖拽素材到画布，或一键用于配音 / 剪辑。</p>
      </div>
    </div>
  );
}
...
function AboutPage({ theme }) {
  const deps = [
    ...
    ["Three.js", "3D 导演台渲染"],
    ["Web Speech API", "本地离线 TTS 配音，无需 API Key"],
    ["Agnes AI", "生图 / 生视频 / 配音 TTS 后端（待接入）"],
  ];
  const shortcuts = [
    ["R / 0", "无限画布重置视图"],
    ["空格 + 左键拖拽", "无限画布平移"],
  ...
  const faq = [
    ["生成失败 / 一直排队？", "检查调度服务器地址（设置→API 设置）是否可达，余额是否充足；云端 GPU 工人繁忙时会排队。"],
    ["离线能用吗？", "本地功能（无限画布、3D 导演台预览、WebSpeech 配音）可用；云端生成 / 云端 TTS 需联网。"],
    ["数据会丢吗？", "工程存于本地 IndexedDB，清理浏览器缓存或站点数据会丢失；建议定期用「导出工程 JSON」备份。"],
    ["ffmpeg 合并失败？", "桌面端已内置 ffmpeg，离线可用；Web 端依赖 ffmpeg.wasm，首次需联网下载内核。"],
  ];
  ...
  return (
    <div style={{ padding: 32, overflowY: "auto", height: "100%", boxSizing: "border-box", color: "var(--text, #e8ecf3)" }}>
      <div style={{ maxWidth: 820, margin: "0 auto" }}>
        ...
        <div style={{ fontSize: 12, color: "var(--accent-2, #5CE1E6)", marginBottom: 16 }}>版本 v0.2.0 · 当前主题 · 烬序深色（已锁定）</div>
        <p style={{ fontSize: 14, color: "var(--text-secondary, #8b95a7)", lineHeight: 1.8 }}>
          烬序 · 影墟是一款面向竖屏短剧创作的桌面工具，帮助创作者从一句话创意出发，
          自动完成分场剧本、分镜画面、视频生成、画布编排、剪辑成片到配音导出的全流程。
          所有工程数据保存在本地 IndexedDB，无需登录即可使用。
        </p>
        ...
        <ol style={{ fontSize: 13, color: "var(--text-secondary, #8b95a7)", lineHeight: 1.9, paddingLeft: 20 }}>
          <li>首页「新建项目」，写下一句话梗概。</li>
          <li>进入「分场剧本」，AI 生成多场分镜（可自定义场次）。</li>
          <li>逐场生成分镜图 / 视频。</li>
          <li>「无限画布」自由排布分镜、机位、人设、剧本片段，连线成镜头顺序。</li>
          <li>「3D 导演台」取景、调灯光天气、截取机位图绑定分镜。</li>
          <li>「视频剪辑」合并成片并添加音频/字幕，「配音」加语音。</li>
          <li>「历史/导出」打包工程，定期备份。</li>
        </ol>
